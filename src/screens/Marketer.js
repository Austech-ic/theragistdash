...
const Marketers = () => {
  ...
  async function createMarketer(e) {
    e.preventDefault();
    setIsLoading(true);
    ...
    try {
      ...
      enqueueSnackbar("Marketer  Created successfully ", { variant: "success" });
      setIsLoading(false);
      closeCreateModal();
    } catch (error) {
  ...
  async function updateMarketer(e) {
    e.preventDefault();
    setIsLoading(true);

    try {
      const response = await api.updateUser(resultId, {
        email: formValue?.email,
        first_name: formValue?.first_name,
        last_name: formValue?.last_name,
      });
      enqueueSnackbar("Marketer Updated successfull", { variant: "success" });
      setIsLoading(false);
      closeCreateModal();
    clearForm()
  ...
  const results = useQuery(["users", page], () => getUser(page), {
                      ...
                      <tr key="_" className="mb-2 hover:bg-light-gray">
                        ...
                        <td className="whitespace-nowrap  flex-item gap-2 bg-white  px-5  border-b-[0.8px] border-[#E4E7EC] text-[14px] leading-[24px] tracking-[0.2px] text-[#1A202C] font-medium text-center ">
                          <div className="flex items-center py-[20px] justify-center gap-2">
                            ...
                            <button onClick={() => ToggleDeleteModal(result)}>
                              <Trash
                                variant="Linear"
                                color="#F05454FF"
                                size="16"
